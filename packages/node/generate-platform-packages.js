 ...
 *
 * Example:
 *   node generate-platform-packages.js 0.7.56 ./artifacts ./platform-packages
 */
...
const PLATFORMS = [
  {
    ...
    os: ['darwin'],
    cpu: ['arm64'],
    description: 'SQLite AI extension for macOS ARM64 (Apple Silicon)',
    binaryName: 'ai.dylib',
    artifactFolder: 'ai-macos',
  },
  {
    name: 'darwin-x86_64',
    os: ['darwin'],
    cpu: ['x64', 'ia32'],
    description: 'SQLite AI extension for macOS x86_64 (Intel)',
    binaryName: 'ai.dylib',
    artifactFolder: 'ai-macos',
  },
  {
    ...
    os: ['linux'],
    cpu: ['arm64'],
    description: 'SQLite AI extension for Linux ARM64 (glibc)',
    binaryName: 'ai.so',
    artifactFolder: 'ai-linux-cpu-arm64',
  },
  {
    name: 'linux-arm64-musl',
    os: ['linux'],
    cpu: ['arm64'],
    description: 'SQLite AI extension for Linux ARM64 (musl)',
    binaryName: 'ai.so',
    artifactFolder: 'ai-linux-musl-cpu-arm64',
  },
  {
    name: 'linux-x86_64',
    os: ['linux'],
    cpu: ['x64', 'ia32'],
    description: 'SQLite AI extension for Linux x86_64 (glibc)',
    binaryName: 'ai.so',
    artifactFolder: 'ai-linux-cpu-x86_64',
  },
  {
    name: 'linux-x86_64-musl',
    os: ['linux'],
    cpu: ['x64', 'ia32'],
    description: 'SQLite AI extension for Linux x86_64 (musl)',
    binaryName: 'ai.so',
    artifactFolder: 'ai-linux-musl-cpu-x86_64',
  },
  {
    name: 'win32-x86_64',
    os: ['win32'],
    cpu: ['x64', 'ia32'],
    description: 'SQLite AI extension for Windows x86_64',
    binaryName: 'ai.dll',
    artifactFolder: 'ai-windows-cpu-x86_64',
  },
];
...
function generatePackageJson(platform, version) {
  return {
    name: `@sqliteai/sqlite-ai-${platform.name}`,
    version: version,
    ...
    repository: {
      type: 'git',
      url: 'https://github.com/sqliteai/sqlite-ai.git',
...
function generateReadme(platform, version) {
  return `# @sqliteai/sqlite-ai-${platform.name}

${platform.description}

**Version:** ${version}

This is a platform-specific package for [@sqliteai/sqlite-ai](https://www.npmjs.com/package/@sqliteai/sqlite-ai).
...
function main() {
  ...
  if (args.length < 3) {
    ...
    console.error('Example: node generate-platform-packages.js 0.7.56 ./artifacts ./platform-packages');
    process.exit(1);
  }
  ...
  if (!/^\d+\.\d+\.\d+$/.test(version)) {
    ...
    console.error('Version must be in semver format (e.g., 0.7.56)');
    process.exit(1);
  }
