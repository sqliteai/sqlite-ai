...
WHISPER_OPTIONS = $(WHISPER) -DWHISPER_BUILD_EXAMPLES=OFF -DWHISPER_BUILD_TESTS=OFF -DWHISPER_BUILD_SERVER=OFF -DWHISPER_RPC=OFF -DWHISPER_USE_SYSTEM_GGML=ON -Dggml_DIR=$(shell pwd)/$(BUILD_GGML)
...
build/whisper.cpp.stamp: build/llama.cpp.stamp
	cmake -B $(BUILD_WHISPER) -DCMAKE_PREFIX_PATH=build/ggml -Dggml_DIR:PATH=$(pwd)/build/ggml/lib/cmake/ggml $(WHISPER_OPTIONS) $(WHISPER_DIR)
